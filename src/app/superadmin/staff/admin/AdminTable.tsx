          ...
          const getList = async () => {
            try {
              const res = await axios.get(
                `${process.env.NEXT_PUBLIC_API_URL}/staffuser/staffuserlist?page=${currentpage}&limit=10&filter=admin&search=${search}&status=${filter}`,
    ...
    <div className=' w-full flex flex-col text-xs bg-white p-4 rounded-md'>
        <div className=' w-full flex md:flex-row flex-col items-center justify-between p-2 gap-2 text-xs'>
            <div className=' flex flex-wrap items-center gap-2 text-xs rounded-md text-black'>
                ...
                <CreateStaffAccount role={'admin'} name={'Admin'}/>
        ...
        <TableHeader>
            ...
            <TableHead className=' w-[120px]'>Full Name</TableHead>
        ...
        <TableBody>
            {staff.map((item, index) => (
                 ...
                 <TableCell className=''>{item.address ? item.address.slice(0,20) : ''}</TableCell>
                 <TableCell className=' w-[100px]'>{item.dateofbirth ? formatDate(item.dateofbirth) : ''}</TableCell>
                 <TableCell className={`${setColor(item.status)} w-[80px]`}>{item.status}</TableCell>
