...
import EditPrograms from '@/components/forms/EditProgram'
...
export default function ProgramTable( ) {
          ...
          const getList = async () => {
            try {
              const res = await axios.get(
                `${process.env.NEXT_PUBLIC_API_URL}/program/getallprogram?page=${currentpage}&limit=10`,
    ...
    const deleteProgram = async () => {
      ...
      try {
           const request = axios.get(`${process.env.NEXT_PUBLIC_API_URL}/program/deleteprogram?id=${id}`, {
              withCredentials: true,
              headers:{
          ...
          const response = await toast.promise(request, {
              ...
              error: 'Error while deleting program',
          });
        ...
        <TableBody>
            {list.map((item, index) => (
                ...
                <TableCell className=' flex items-center gap-2'>
                  ...
                  <EditPrograms name={item.name} id={item.id}/>

                <Dialog open={modal} onOpenChange={setModal}>
                    <DialogTrigger onClick={() => {setId(item.id), setName(item.name)}} className=' danger-btn'>
                        <img src='/icons/info-circle.png' width={15} height={15}/>Delete
                    </DialogTrigger>
                    <DialogContent className=' text-xs'>
                        <DialogHeader>
                        <DialogTitle className=' text-lg text-red-600'>Delete Program</DialogTitle>
                        ...
                        <div className=' w-full flex flex-col'>
                          ...
                          <p>Program: {name}</p>
                            ...
                            <div className=' w-full flex items-end justify-end mt-6'>
                                <button onClick={() => deleteProgram()} className=' action-danger'>
