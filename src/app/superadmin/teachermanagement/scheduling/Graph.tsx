...
export default function Graph() {
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(() => {
          const getList = async () => {
            try {
              const res = await axios.get(
                `${process.env.NEXT_PUBLIC_API_URL}/staffuser/getteacherlist?page=0&limit=10000&filter=teacher&search=${search}&status=`,
                { withCredentials: true }
              );
  ...
  return (
    ...
    <div className=' w-full'>
        <div className=' flex flex-col gap-4 w-full h-auto bg-white p-6 rounded-md text-xs'>
          <p className=' text-sm font-semibold'>Teacher Info</p>
          <div className=' flex flex-col gap-2'>
            <p className=' '>Name: <span className='font-medium'>{info?.fullname}</span></p>
            <p>Address: {info?.address}</p>
            <p>Date of birth: {formatDate(info?.dateofbirth || '')}</p>
            <p>Email: {info?.email}</p>
          </div>

          <div className=' flex items-center gap-2 text-[.6rem]'>
            <a href={`/superadmin/teachermanagement/scheduling/teacherschedule?id=${id}`} className=' action-btn'><File size={10}/>Download / Print</a>
          </div>
    ...
    <div className=' w-full  h-full grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-4'>
              ...
              return (
                ...
                >
                  <div className=' flex flex-col h-[500px]'>
                    ...
                    <CreateScheduleTeacher teacherid={id} day={day} >
        ...
        <div className=' flex flex-col gap-1 p-4 w-full h-full bg-white rounded-md'>
            ...
            <p className=' text-sm font-medium mb-2'>Teacher List</p>
            {teacher.map((item, index) => (
                <p key={item.id} onClick={() => setId(item.id)} className={`cursor-pointer marker:${id === item.id ? ' bg-pink-600 text-white' : ' bg-pink-50'} p-2 rounded-sm text-xs`}>{item.fullname}</p>
            ))}
