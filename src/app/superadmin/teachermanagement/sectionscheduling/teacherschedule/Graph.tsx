...
export default function Graph() {
    ...
    const info = schedule.find((item) => item.id === getId )
          ...
          const getList = async () => {
            try {
              const res = await axios.get(
                `${process.env.NEXT_PUBLIC_API_URL}/schedule/getschedulebysection?section=${getId}`,
  ...
  return (
    ...
    <div id='Schedule' className='w-[1240px] bg-white text-xs p-4' ref={contentRef}>
      <div className=' w-full flex items-center justify-center gap-2'>
        ...
        <p className=' font-medium mt-6'>Section: {schedule[0]?.section}</p>
