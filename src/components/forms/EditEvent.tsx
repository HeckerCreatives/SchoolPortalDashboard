...
import { createEvents, CreateEvents, editEvents, EditEvents } from '@/validations/validatiion'
...
export default function EditEvent( prop: Props) {
    ...
    const [file, setFile] = useState<File>()
      ...
      const {
        ...
        resolver: zodResolver(editEvents),
        defaultValues: {
            title: prop.title,
    ...
    //edit events
    const onSubmit = async (data: EditEvents) => {
        setLoading(true)
        router.push('?state=true')
        try {
             const request = axios.post(`${process.env.NEXT_PUBLIC_API_URL}/event/editevent`,{
                title: data.title,
        ...
        <DialogContent className=' text-xs max-h-[700px] overflow-y-auto '>
            ...
            <DialogDescription className=' text-xs'>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className={` w-full flex flex-col`}>
                    ...
                    <label htmlFor="firstname" className=' mt-2'>Event Date</label>
                    <input type="date" placeholder='Title' className=' input-primary' {...register('date')}/>
