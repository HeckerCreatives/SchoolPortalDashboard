...
export default function EditScheduleTeacher(prop: Props) {
    ...
    const findSubject = subject.find((item) => item.name === prop.subject)
    const findSection = section.find((item) => item.name === prop.section)
    ...
    useEffect(() => {
      const timeoutId = setTimeout(() => {
        const getList = async () => {
          try {
            const res = await axios.get(
              `${process.env.NEXT_PUBLIC_API_URL}/section/getsectionbygradelevel?level=${selectedGradeLevel}`,
              { withCredentials: true }
            );
  ...
  const onSubmit = async (data: UpdateSchedule) => {
      ...
      try {
          const request = axios.post(`${process.env.NEXT_PUBLIC_API_URL}/schedule/editschedule`,{
            schedule: prop.id,
          ...
          const response = await toast.promise(request, {
              ...
              error: 'Error while updating schedule',
          });
  ...
  useEffect(() => {
    if(prop.section || findSection || findSubject){
      reset({
  ...
  },[reset, prop.section, findSection, findSubject, prop])
