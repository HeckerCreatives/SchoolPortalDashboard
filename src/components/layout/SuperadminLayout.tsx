...
import { superadmin } from '@/routes/route'
            ...
            <div className=" relative z-10 flex-1 mt-4 overflow-y-auto">
              <nav className=" flex flex-col px-2 text-sm lg:px-6 ">
                ...
                {superadmin.map((item, index) => (
                    ...
                    <Collapsible key={index} className="group/collapsible p-0">
                        <CollapsibleTrigger className={` flex items-center rounded-md p-3 w-full justify-between ${path.includes(item.path) ? ' bg-pink-50 text-pink-600' : 'text-zinc-500'}`}>
                            <p className={` flex items-center gap-2 rounded-md cursor-pointer `}>{item.icon}{item.name}</p>
                            
                            <ChevronDown size={20} className="ml-auto transition-transform duration-200 group-data-[state=open]/collapsible:rotate-180 bg-pink-100 p-1 rounded-full"/>
                        </CollapsibleTrigger>
                        <CollapsibleContent className={`flex flex-col text-zinc-500 gap-3 pl-8 group-data-[state=open]/collapsible:py-2`}>
                        {item.subpath.map((item, index) => (
                            <a key={index} href={item.path} className={` ${path === item.path && 'text-pink-600'}`}>{item.name}</a>
                        ))}
                        </CollapsibleContent>
                    </Collapsible>
                    )}
                ...
                >
                  ...
                  <nav className=" flex flex-col gap-2 px-2 text-sm font-medium">
                      {superadmin.map((item, index) => (
                        ...
                        <Collapsible key={index} className="group/collapsible p-0">
                            <CollapsibleTrigger className={` flex items-center rounded-md p-3 w-full justify-between ${path.includes(item.path) ? ' bg-pink-50 text-pink-600' : 'text-zinc-500'}`}>
                                <p className={` flex items-center gap-2 rounded-md cursor-pointer text-xs `}>{item.icon}{item.name}</p>
                                
                                <ChevronDown size={20} className="ml-auto transition-transform duration-200 group-data-[state=open]/collapsible:rotate-180 bg-pink-100 p-1 rounded-full"/>
                            </CollapsibleTrigger>
                            <CollapsibleContent className={`flex flex-col text-zinc-500 gap-3 pl-8 group-data-[state=open]/collapsible:py-2`}>
                            {item.subpath.map((item, index) => (
                                <a key={index} href={item.path} className={` text-xs ${path === item.path && 'text-pink-600'}`}>{item.name}</a>
                            ))}
                            </CollapsibleContent>
                        </Collapsible>
                        )}
